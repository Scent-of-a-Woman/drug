	...
	<div class="orderdetail">
		<div class="process">
			<el-breadcrumb separator-class="el-icon-minus">
				...
				<el-breadcrumb-item :to="{ path: '/order/take' }">配送订单</el-breadcrumb-item>
				...
				<el-tabs v-model="activeName">
					<el-tab-pane label="订单详情" name="first">
						<div class="orderdetail_content">
							...
							<div class="huadong">
								...
								<div class="info_lists info_list">
									<ul class="list_head" v-for="(item,index) in drug_data">
										<li>{{item.drugCode}}</li>
										<li :title="item.drugName">{{item.drugName}}</li>
										...
										<li class="trace">
											...
											maxlength="12" v-model="item.validDate" placeholder="格式 xxxx-xx-xx">
										</li>
										<li class="trace">
							...
							<div class="sure_info" v-if="this.orderStatus==1?false:true">
								
								<span  @click="takend">配送完成</span>
							</div>
						</div>
					</el-tab-pane>
...
<script>
	...
	export default { 
		...
		created(){
			...
			this.cfId=this.$route.query.cfId //处方id
			this.requerstData()
			this.cfdetail()
		},
		methods: { 
...
.user_name p,.user_phone p,.user_address p{
	height: 30px;
	line-height: 30px;
	color: #a4a4a4;
	font-size: 14px;
}
.user_name input,.user_phone input,.user_address input{
	height:25;
	line-height: 25px;
	border:1px solid #dcdcdc;
	padding-left:10px;
