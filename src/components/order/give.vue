	...
	<div class="take">
		<div class="process">
			<el-breadcrumb separator-class="el-icon-minus">
				...
				<el-breadcrumb-item :to="{ path: '/order/give' }">配送订单</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		<!-- 内容 -->
		...
		<div class="take_lists">
			<div class="info_list">
				<ul class="list_head">
					<li>处方编号</li>
					<li>配送药企</li>
					<li :style="{'width':id==5?'15%':'12%'}">联系方式</li>
					<li>开方时间</li>
					<li :style="{'width':id==5?'19%':'12%'}">配送状态</li>
			...
			<div class="info_lists info_list">
				<ul class="list_head" v-for="(item,index) in data">
					...
					{{item.gysName}}</li>
					<li :style="{'width':id==5?'15%':'12%'}">{{item.phone}}</li>
					<li>{{item.createTime}}</li>
					<li :style="{'color':statusText[item.status][0],'width':id==5?'19%':'12%'}">{{statusText[item.status][1]}}</li>
					<li v-if="id==5?false:true">
						<router-link :to="{path:'./orderdetail',query:{id:item.id,cfId:item.cfId}}"><i class="el-icon-search icon_turn"></i></router-link>     
					</li>
				</ul>
			</div>
		</div>
